...
- Code-touching roles allow Serena symbol/reference/declaration tools and CodeGraph node/callers/callees/explore tools. Availability does not require using every tool. AGENTS.md §10 and the active yct-ca skill own query selection and prior-art handling; a write packet carries verified candidates or the source-worktree query still needed.
